...
are owned by ``vadim_prod`` and the directory is 0700 (it holds secrets). A
cross-user SQLite bind-mount would mean weakening those permissions, which we
refuse to do.

So this tiny stdlib HTTP service runs **as ``vadim_prod``** (same user that owns
...
conductor then claims the job and runs **Claude Code** (profile ``full_stack_sm``, the
...
DEFAULT_PROFILE = os.environ.get("BRIDGE_DEFAULT_PROFILE", "full_stack_sm")
