...
0. **НИКОГДА не запускай систему (full_stack_sm / seo_sm / marketing_sm) сам.**
...
|---|---|---|
| SEO, search ranking, SERP, crawl, indexation, sitemap, robots, canonical, hreflang, Core Web Vitals, keywords, backlinks | `seo_sm` | `/seo-audit` |
| campaign, ads, paid media, funnel, email/CRM, social, content, copywriting, positioning, GTM, launch, brand | `marketing_sm` | `/mkt-campaign` |
| security audit, vulnerability, pentest, OWASP, RLS/auth/secrets review | `security_sm` | (agents; or `/sm-verify`) |
| code, build, refactor, bug fix, deploy, technical analysis, docs — **the default** | `full_stack_sm` | `/sm-feature`, `/sm-verify` |

If genuinely ambiguous, ask ONE question; otherwise default to `full_stack_sm`.
Examples: "подними нам трафик из Google" → `seo_sm`; "запусти рекламную кампанию /
сделай контент-план / email-цепочку" → `marketing_sm`; "проверь на уязвимости /
проаудь RLS" → `security_sm`; "сделай фичу / почини баг / задеплой" → `full_stack_sm`.
...
- **C. Autonomous conductor (A→Z projects).** Do NOT switch globally. Set the
  ...
  (`conductor/sql/schema.sql` + `src/core/profiles.ts`). NULL = `full_stack_sm`.

**Deterministic helpers — CALL these, don't classify in your head** (your model
is small; the scripts are the reliable backbone):
- `route-profile.sh "<task text>"` → prints `full_stack_sm|seo_sm|marketing_sm|security_sm|ambiguous`.
...
3. Map his reply with `…/route-profile.sh --num <n>` (1=full_stack_sm, 2=marketing_sm, 3=seo_sm,
   4=security_sm); accept the profile name too. Unrecognized reply → re-ask once.
...
2. Повторяй до **4 раз**. Если и после этого `error_max_turns` — задача не для
   ...
   full_stack_sm вручную в новом топике» — и приложи `session_id`, чтобы работа не
...
7. **Ты никогда не запускаешь систему (full_stack_sm / seo_sm / marketing_sm) сам.**
