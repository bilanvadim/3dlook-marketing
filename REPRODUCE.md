...
2. **Install upstream hermes-agent** into `~/.hermes/hermes-agent` — see [`hermes-agent/SETUP.md`](hermes_agent/SETUP.md) §2.
...
4. **Work the printed TODO list** — fill `.env`/`config.yaml`, `hermes auth`, enroll MTProto + userbot sessions, enable systemd units + cron, run `claude-code-agent/DEV/switch-profile.sh full_stack_sm`, install the codebase-memory binary.
...
## Detailed guides
- Hermes side: [`hermes_agent/SETUP.md`](hermes_agent/SETUP.md) · [`CONFIG.md`](hermes_agent/CONFIG.md)
- Claude Code side: [`claude_code/INSTALL.md`](claude_code/INSTALL.md) · [`DEV/SYSTEMS.md`](claude_code/DEV/SYSTEMS.md)
...
Подробности — `opencode_agent/README.md`.
