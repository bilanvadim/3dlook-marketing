...
47821/47822): `127.0.0.1` на этом VPS общий для обоих аккаунтов — с его портами этот
Hermes ушёл бы в ЕГО прокси и его ключи.

И **ставь `server.apiKey`** в конфиге прокси, когда будешь его поднимать. По умолчанию
там `null`, а loopback общий: проверено с другого аккаунта — `curl
http://127.0.0.1:47821/v1/models` отдавал `200`, то есть любой локальный процесс мог
тратить чужие ключи провайдеров. Значение — то же, что Hermes подставляет для
custom-провайдера (`HERMES_CUSTOM_127_0_0_1_47832_API_KEY` в `~/.hermes/.env`), и его же
надо положить OpenCode в `~/.local/share/opencode/auth.json`. У Сергея это уже закрыто:
без ключа теперь `401`.
