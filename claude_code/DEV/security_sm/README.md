# Security system (`ai-agents-sec`)

Claude Code marketplace for the **security** profile. Sibling of `full_stack_sm`
(dev), `seo_sm` (SEO), `marketing_sm` (marketing). Only one profile is active at a
...
- `security-scan` — wraps external `ecc-agentshield`; our `hermes_agent/ops/
