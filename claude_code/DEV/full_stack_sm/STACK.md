...
- **All other projects → plain PostgreSQL** per the map below. Status: SimplifyEU (Auth+Data+RLS — good pilot), ASCoFacade (Auth+Data, light storage/realtime), smiro.dev (trivial). The conductor itself does NOT use this Postgres — its `ho_*` orchestration state is **local SQLite via `better-sqlite3`**, independent of whatever DB the built apps use. Not libSQL and not Turso: that driver leaked a connection per transaction (5.4 GB while polling an empty queue) and `dbPath()` now rejects remote schemes.
