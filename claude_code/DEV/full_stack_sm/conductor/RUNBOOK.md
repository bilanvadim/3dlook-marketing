...
```bash
cp hermes_agent/ops/systemd/hermes-conductor.service ~/.config/systemd/user/
...
ExecStart = `hermes_agent/ops/conductor-run.sh` (defaults `DATABASE_URL` to
...
sqlite3 ~/.hermes/ho.db "insert into ho_jobs(kind,title,prompt,profile,work_dir,max_turns,max_wall_secs)
  values('feature','<title>','<prompt>','full_stack_sm','/home/vadim_prod/workspaces/<proj>',60,7200);"
```

- **`profile` MUST be a long name** — `full_stack_sm | seo_sm | marketing_sm | security_sm`
  (matches `claude_code/DEV/profiles/<profile>.json`). The worker logs
  `job N profile='full_stack_sm' → 11 plugin(s)` when it resolves.
...
(`profile in ('dev','seo',...)`) — inserting `'full_stack_sm'` then fails the CHECK, and
inserting `'dev'` resolves **0 plugins** (`no manifest for 'dev'`). Rebuild it
(the current `schema.sql` has the correct `'full_stack_sm'…` constraint):
...
sqlite3 ~/.hermes/ho.db < claude_code/DEV/full_stack_sm/conductor/sql/schema.sql
systemctl --user start hermes-conductor.service
```

## Live smoke — what was verified (2026-07-18)

A throwaway `feature` job proved end-to-end: claim → **full_stack_sm → 11 plugins** →
