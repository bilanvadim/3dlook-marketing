...
> Source of truth for a project lives HERE (conductor SQLite state + scratchpad files in the workspace), never inside Hermes. If Hermes restarts, it rehydrates everything from the surfaces below. Hermes holds only its Telegram thread.
...
## Surfaces Hermes consumes (local SQLite — direct SQL; better-sqlite3, no Turso)
...
→ Authoritative project context = **scratchpad files + SQLite** (Fullstack side). Hermes = thin relay. Hermes' own long-term cross-project memory (Obsidian Graph) is separate and added later.
