...
> Source of truth for a project lives HERE (conductor Postgres + scratchpad files in the workspace), never inside Hermes. If Hermes restarts, it rehydrates everything from the surfaces below. Hermes holds only its Telegram thread.
...
## Surfaces Hermes consumes (plain Postgres — direct SQL via a read-only role; no Supabase/PostgREST)
- **`hc_project_status`** (view) — per job: `job_status`, `percent`, `total_steps`, `done_steps`, `open_questions`, `open_escalations`, `last_activity`. The single read for a Telegram status update.
- **`hc_steps`** — per-step state: `status`, `attempts`, `score`, `acceptance` (with evidence), reviewer/runtime reports. Drives "% done" and "what's happening now".
- **`hc_questions`** — the async interview channel. Hermes reads `status='open'` rows, relays to Telegram, writes answers via `hc_answer_question(id, text)`. When the last open question is answered, the job leaves `awaiting-input` and resumes.
- **`hc_escalations`** — human decisions (approve/deny/abort) for gated actions / blocks.
- **`hc_jobs`** — the job + `resume_session_id` (durable resume).

## The awaiting-input (interview) flow — where context lives
1. `product-architect` needs human input → writes its questions + partial state to **scratchpad files** in the workspace, inserts rows into **`hc_questions`** (status=open), sets job `awaiting-input`. **The Claude Code run ends** (no idle session, no token burn).
2. Hermes reads open questions → asks the human in Telegram → writes answers back with `hc_answer_question(...)`.
...
→ Authoritative project context = **scratchpad files + Postgres** (Fullstack side). Hermes = thin relay. Hermes' own long-term cross-project memory (Obsidian Graph) is separate and added later.

## Per-step execution (the deterministic loop)
For each ready step (`hc_next_step` picks the next `pending` step whose `depends_on` are all `done`):
