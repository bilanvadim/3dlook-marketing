# Security system (`security-sm`)

Claude Code marketplace for the **security** profile. Sibling of `dev-sm`
(dev), `seo-sm` (SEO), `marketing-sm` (marketing). Only one profile is active at a
