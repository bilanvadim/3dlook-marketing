 ...
 *
 * A job's `profile` (ho_jobs.profile: dev-sm|seo-sm|marketing-sm|security-sm) selects a
...
// profiles live at agents-ai/telegram-bot-agent/claude-code-agent/DEV/profiles (four levels up: core→src→conductor→dev-sm→DEV)
...
export function resolveProfilePlugins(profile: string | null | undefined): LocalPlugin[] {
  const name = (profile && profile.trim()) || 'dev-sm';
