...
// Autonomous headless worker: there is no human to approve permission prompts in a
// conductor run, so bypass them entirely. This lets the executor freely use Bash, edits,
// and the codebase-memory MCP tools (bundled in hermes-core, loaded via options.plugins).
// bypassPermissions REQUIRES allowDangerouslySkipPermissions:true (SDK contract).
// Safety net = the SYS prompt ("never run prod-affecting commands without an explicit ask")
// + single-step scoping. ⚠️ guard.py is a project-hook of the dev-sm profile, NOT of the
// target workDir, so it does NOT fire here — prod-op protection is prompt-level only.
...
async function runQuery(prompt: string, workDir: string, plugins: LocalPlugin[], maxTurns = 150): Promise<{ text: string; ok: boolean }> {
  ...
  const stream = query({
    prompt,
    options: { settingSources: ['project'], plugins, permissionMode: 'bypassPermissions', allowDangerouslySkipPermissions: true, systemPrompt: SYS, cwd: workDir, maxTurns } as any,
  });
