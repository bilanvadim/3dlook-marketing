...
Claude Code marketplace for the **SEO** profile. Sibling of `dev-sm` (dev)
and `marketing-sm` (digital marketing). Only one profile is active at a time —
switch with `../switch-profile.sh seo` (see `../profiles/`).

The `seo` profile also loads the shared base (`hermes-core`, `hermes-verify` from
`dev-sm`), so orchestration, scratchpad handoff, session-handoff and
...
`seo-sm/CLAUDE.md` is the SEO orchestrator doc (analogous to
