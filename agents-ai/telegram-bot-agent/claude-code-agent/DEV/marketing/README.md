# Digital marketing system (`marketing`)

Claude Code marketplace for the **marketing** profile. Sibling of `dev`
(dev) and `seo` (SEO). Only one profile is active at a time — switch with
`../switch-profile.sh marketing` (see `../profiles/`).

The `marketing` profile also loads the shared base (`hermes-core`,
`hermes-verify` from `dev`), so orchestration, scratchpad handoff,
...
`marketing/CLAUDE.md` is the marketing orchestrator doc (analogous to
`dev/CLAUDE.md`): work cycle, who-to-call table, hard rules
