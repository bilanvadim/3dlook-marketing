...
- **`ho_questions`** — the async interview channel. Hermes reads `status='open'` rows, relays to Telegram, writes answers with `store.answerQuestion(id, text)` (or a direct `update ho_questions set answer=…, status='answered'` + release, see below). When the LAST open question is answered the job returns to `queued` and a worker picks it up again. ⚠️ `ho_answer_question(id, text)` was a Postgres stored procedure and does NOT exist in the SQLite schema — writing answers without releasing the job leaves it parked in `awaiting-input` forever.
...
2. Hermes reads open questions → asks the human in Telegram → writes answers back with `store.answerQuestion(...)`, which releases the job when the last one is answered.
