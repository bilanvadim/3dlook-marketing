...
- **`supabase.smiro.dev` → Supabase Studio** — already live: Traefik → `oauth2-proxy-supabase` (GitHub OAuth, user `@GH_OWNER@` only) → `supabase-studio:3000`; API paths `/rest /auth /storage` → kong. Don't disturb (way2buy depends on it).
