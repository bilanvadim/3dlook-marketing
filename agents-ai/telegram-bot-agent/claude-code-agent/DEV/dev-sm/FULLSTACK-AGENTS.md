...
│   ├── hermes-scout/     → solution-evaluator (/sm-evaluate конкретного решения)
...
3. **solution-evaluator НИКОГДА не устанавливает** оценённое. Только вердикт.
...
│  Claude Code plugin marketplace: 9 агентов,                    │
...
└── plugins/
    ...
    └── hermes-scout/                ← solution-evaluator (/sm-evaluate; report-only)
```
...
[человек или cron]
   │  ставит задачу
   ▼
Hermes / sqlite3 ──insert into ho_jobs (status=queued)──►  ho.db
   ▼
conductor/ (воркер)
...
5. **Triggers.** Задачи ставятся прямым `insert into ho_jobs` (Hermes делает это через скилл `vps-orchestration`). n8n-диспетчер удалён 2026-08-09: обе его ноты писали в Postgres, которого нет с переезда на libSQL, поэтому вебхук отвечал `{"queued": true}` в пустоту, а дневной cron scout не создал ни одной задачи за всё время. Вебхук был вдобавок без аутентификации, а тело запроса задаёт `work_dir` для исполнителя с `bypassPermissions` — чинить такое имело смысл только при живом потребителе, а его нет.
6. **Первый прогон.** Поставить тестовую задачу (`insert into ho_jobs`) с небольшим `max_turns` и смотреть статусы в `ho_jobs` / `ho_runs`. Проверить, что `resume_session_id` проставляется, и что пауза по лимиту переводит job в `deferred`, а не в `failed`.
