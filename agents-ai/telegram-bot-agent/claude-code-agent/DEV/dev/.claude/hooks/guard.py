...
# (safety-guard ruleset, adapted from ECC pattern — see security; kept conservative)
BLOCK = [
    (r"\brm\s+-[a-z]*r[a-z]*f?\s+(-[a-z]+\s+)*(/\s*$|/\s|~|\$HOME|\.git|\*)", "recursive delete of root/home/.git/glob"),
    # The rule above only fires for `/` at end-of-argument, `~`, `$HOME`, `.git` or a glob — so
    # `rm -rf /home/<user>/workspaces` sailed straight through it: that is a slash followed by a
    # LETTER, which neither `/\s*$` nor `/\s` matches. An autonomous run could therefore delete any
    # named absolute path with the guard reporting nothing. Verified before the fix: exit 0.
    #
    # `../` is here for the same reason — a recursive delete reaching above the work_dir is outside
    # what a single-step agent was asked to touch.
...
# NO blanket "rm -r" entry here, deliberately. It was added and removed the same hour: the harness
# asserts `rm -rf ./build` and `rm -rf node_modules` are ALLOWED, and it is right — those are
# everyday operations, and in a HEADLESS run an ASK is effectively a DENY, so gating them would
# break routine dependency cleanup. A recursive delete that leaves the work_dir is a BLOCK above;
# anything inside it is gated by the conductor's own ASK_PATTERNS over Telegram, where there is a
# human who can actually answer.
ASK = [
