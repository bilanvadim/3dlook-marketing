# Digital marketing system (`marketing-sm`)

Claude Code marketplace for the **marketing** profile. Sibling of `dev-sm`
(dev) and `seo-sm` (SEO). Only one profile is active at a time — switch with
`../switch-profile.sh marketing` (see `../profiles/`).

The `marketing` profile also loads the shared base (`hermes-core`,
`hermes-verify` from `dev-sm`), so orchestration, scratchpad handoff,
...
`marketing-sm/CLAUDE.md` is the marketing orchestrator doc (analogous to
`dev-sm/CLAUDE.md`): work cycle, who-to-call table, hard rules
