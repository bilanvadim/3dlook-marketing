# Systems (profiles) — dev-sm · seo-sm · marketing-sm · security-sm · marketing_vb · marketing_vb_sm (+ sandbox-sm)
...
system's agents, skills and commands. Six are working systems; `sandbox-sm` is
...
| **marketing_vb_sm** | the MIX: Vadim's teams + Sergiy's marketing + base | `dev-sm`(base) + `marketing-sm` + `marketing_vb` + `marketing_vb_sm` | `/vbsm-campaign` | mvb 28 + mkt 5 |
...
./switch-profile.sh marketing-sm     # switch (dev-sm | seo-sm | marketing-sm | security-sm | marketing_vb | marketing_vb_sm | sandbox-sm)
```
