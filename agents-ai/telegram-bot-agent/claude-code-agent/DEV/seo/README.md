...
Claude Code marketplace for the **SEO** profile. Sibling of `dev` (dev)
and `marketing` (digital marketing). Only one profile is active at a time —
switch with `../switch-profile.sh seo` (see `../profiles/`).

The `seo` profile also loads the shared base (`hermes-core`, `hermes-verify` from
`dev`), so orchestration, scratchpad handoff, session-handoff and
...
`seo/CLAUDE.md` is the SEO orchestrator doc (analogous to
