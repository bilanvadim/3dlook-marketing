...
Скопируйте `agents-ai/telegram-bot-agent/hermes-agent/mem0.json.example` в `~/.hermes/mem0.json` и задайте
Gemini API key в `~/.hermes/.env`:

```bash
cp agents-ai/telegram-bot-agent/hermes-agent/mem0.json.example ~/.hermes/mem0.json
...
(`cp agents-ai/telegram-bot-agent/hermes-agent/.env.example ~/.hermes/.env` и заполнить значения) — либо
...
```bash
cp agents-ai/telegram-bot-agent/hermes-agent/SOUL.md ~/.hermes/SOUL.md
```
...
cp /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/refresh.py      ~/.hermes/model-router/
cp /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/router_lib.py   ~/.hermes/model-router/
cp /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/model-strength.json ~/.hermes/model-router/
```

### systemd timer (запуск в 07:00 каждый день):

```bash
# Юниты уже в репо: agents-ai/telegram-bot-agent/hermes-agent/ops/systemd/model-router-refresh.{service,timer}
cp /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/systemd/model-router-refresh.service ~/.config/systemd/user/
cp /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/systemd/model-router-refresh.timer   ~/.config/systemd/user/
...
cp /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/systemd/hermes-gateway.service ~/.config/systemd/user/
...
```bash
# Установить crontab:
crontab -l 2>/dev/null | { cat; echo "# Hermes conductor monitor — push escalations to Telegram"; echo "*/5 * * * * /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/conductor-monitor.sh >> ~/.hermes/conductor-monitor.log 2>&1"; } | crontab -

# Инициализация (отметить текущее состояние как уже-виденное — без спама):
/srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/conductor-monitor.sh --init
```
...
cp /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/ops/vault-sync.sh ~/.hermes/vault-sync.sh
...
cp -r /srv/$USER/ai-agents-config/agents-ai/telegram-bot-agent/hermes-agent/skills/vps-orchestration \
...
cp agents-ai/telegram-bot-agent/hermes-agent/ops/hermes-update.py              ~/.hermes/hermes-update.py
cp agents-ai/telegram-bot-agent/hermes-agent/ops/systemd/hermes-update.service ~/.config/systemd/user/
cp agents-ai/telegram-bot-agent/hermes-agent/ops/systemd/hermes-update.timer   ~/.config/systemd/user/
systemctl --user daemon-reload
...
- **После апдейта пере-применяет file-tool guard** (`ops/apply-file-tool-guard.py`) — патч
  ...
  `python3 agents-ai/telegram-bot-agent/hermes-agent/ops/apply-file-tool-guard.py`.
...
|---|---|---|
| `config.yaml` | главный конфиг Hermes | `agents-ai/telegram-bot-agent/hermes-agent/CONFIG.md` (reference) |
...
| `model-router/refresh.py` | ежедневный выбор модели | `agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/refresh.py` |
| `model-router/router_lib.py` | общие хелперы (API, Telegram, config edit) | `agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/router_lib.py` |
| `model-router/model-strength.json` | веса формулы + сила моделей | `agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/model-strength.json` |
...
| `vault-sync.sh` | авто-коммит вики | `agents-ai/telegram-bot-agent/hermes-agent/ops/vault-sync.sh` |
| `skills/autonomous-ai-agents/vps-orchestration/` | операционная политика | `agents-ai/telegram-bot-agent/hermes-agent/skills/vps-orchestration/` |
...
cp agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/refresh.py      ~/.hermes/model-router/
cp agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/router_lib.py   ~/.hermes/model-router/
cp agents-ai/telegram-bot-agent/hermes-agent/ops/model-router/model-strength.json ~/.hermes/model-router/
cp agents-ai/telegram-bot-agent/hermes-agent/ops/vault-sync.sh                 ~/.hermes/vault-sync.sh
cp agents-ai/telegram-bot-agent/hermes-agent/ops/hermes-update.py              ~/.hermes/hermes-update.py
cp -r agents-ai/telegram-bot-agent/hermes-agent/skills/vps-orchestration       ~/.hermes/skills/autonomous-ai-agents/
