...
ADAPTER_PY = os.path.join(GATEWAY, os.pardir, "plugins", "platforms", "telegram", "adapter.py")
...
def _patch_file(path, edits):
    ...
    for edit in edits:
        ...
        present_test = edit[3] if len(edit) > 3 else replacement
...
def main():
    ...
    # 3. run.py — three inserts
    r = _patch_file(RUN_PY, [
        ("dispatch", DISPATCH_ANCHOR, DISPATCH_INSERT + DISPATCH_ANCHOR),
        ("intercept-primary", S1_ANCHOR, S1_ANCHOR + S1_INSERT),
        ("intercept-followup", S2_ANCHOR, S2_ANCHOR + S2_INSERT),
        ("forward-picker", FWD_ANCHOR, FWD_INSERT + FWD_ANCHOR),
    ] + BUSY_ACK_EDITS)
    if isinstance(r, list):
    ...
    r = _patch_file(ADAPTER_PY, [
        ("panel-callback", ADAPTER_ANCHOR, ADAPTER_ANCHOR + ADAPTER_INSERT),
        ("inline-query", ADAPTER_IQ_ANCHOR, ADAPTER_IQ_ANCHOR + ADAPTER_IQ_INSERT),
        ("fwd-provenance", ADAPTER_FWD_ANCHOR, ADAPTER_FWD_ANCHOR + ADAPTER_FWD_INSERT),
    ])
    if isinstance(r, list):
