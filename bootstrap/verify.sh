...
# The embedded Python checks print their own [OK]/[WARN]/[FAIL] lines. Until 2026-09-18
# those went straight to stdout and never reached the counters, so the verdict could say
# "SYSTEM READY" directly under three [FAIL] lines from the barrier check. Every Python
# block is captured and fed through here instead; a block that crashes is a FAIL too,
# because a check that dies prints nothing and would otherwise read as a pass.
tally() {  # tally <label> <exit status> <captured output>
  local line
  ...
  [ "$2" = 0 ] || fail "$1 check crashed (exit $2) — its result above is incomplete"
}
...
if [ ! -f "$SRC" ]; then
  ...
  _py_out="$(python3 - "$SRC" "$HOME/.hermes/.env" <<'PYSEC'
...
_py_out="$(python3 - "$HOME/.hermes/config.yaml" "$REPO" <<'PYBAR'
...
al = [str(a) for a in (cfg.get("command_allowlist") or [])]
# Exact strings Vadim approved on 2026-09-18 — see the file's header. Anything else is a
# standing bypass, most likely from the 'Always' button. A missing file approves nothing.
try:
    with open(f"{repo}/hermes_agent/ops/approved-command-allowlist.txt", encoding="utf-8") as f:
        approved = {ln.strip() for ln in f if ln.strip() and not ln.lstrip().startswith("#")}
except OSError:
    approved = set()
    print("[WARN] hermes_agent/ops/approved-command-allowlist.txt unreadable — no entry counts as approved")
unknown = [a for a in al if a not in approved]
if unknown:
    print(f"[FAIL] command_allowlist has {len(unknown)} unapproved standing bypass(es): {unknown[:3]} — written by the 'Always' button?")
elif al:
    print(f"[OK]   command_allowlist holds only the {len(al)} approved entries")
else:
    print("[OK]   command_allowlist empty")
stale = sorted(approved - set(al))
if stale:
    print(f"[WARN] approved allowlist entries no longer in config.yaml (drop them from the file): {stale[:3]}")
deny = [str(d) for d in (ap.get("deny") or [])]
# The barrier is only real if it matches the tree that actually holds the code.
# Since 2026-09-12 the barrier is WRITE-only (ops/hermes-read-carveout.py): Hermes that
# cannot read outbound-pipeline.py improvises the pipeline instead of running it, which is
# what threw away 218 hand-made messages that day. So reads must pass and every write
# vector must still hit a deny glob. These probes used to demand the opposite for grep,
# which made this check fail on a healthy box and pass on one whose carve-out reverted.
probes = [
    (f"cd {repo} && grep -rn x .",              False, "grep inside the repo (read carve-out)"),
    ("cd ~/3dlook-marketing && grep -rn x .",   False, "grep via the ~ spelling (read carve-out)"),
    (f"find {repo}/marketing_vb/scripts -name '*.py'", False, "find inside the repo (read carve-out)"),
    (f"sed -i 's/a/b/' {repo}/docs/x.md",       True,  "in-place edit of repo docs"),
    ("sed -i 's/a/b/' ~/3dlook-marketing/docs/x.md", True, "in-place edit via the ~ spelling"),
    (f"echo x > {repo}/marketing_vb/scripts/x.py", True, "redirect into the repo"),
    (f"cat y | tee {repo}/docs/x.md",           True,  "tee into the repo"),
    (f"vim {repo}/docs/x.md",                   True,  "editor on repo files"),
...
if not bad:
    print(f"[OK]   {len(deny)} deny globs block every write vector into the repo and still allow reads/dispatch/git/handoff")
PYBAR
)"; tally "barrier" "$?" "$_py_out"

# ── the model selector ───────────────────────────────────────────────────────
hdr "Profiles"
_py_out="$(python3 - "$REPO" "${HO_DB:-$HOME/.hermes/ho.db}" <<'PYPROF'
...
_py_out="$(python3 - "$HOME/.hermes/config.yaml" <<'PYMODEL'
import sys, yaml
