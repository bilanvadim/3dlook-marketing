...
3. **Do not use**: em dash anywhere, "It's not just X, it's Y", triple parallelisms, banned words, or the word-level bans in `brand-assets/content-strategy/terminology-guardrails.md`. Four rules from its 2026-09-14 sync that a post reaches for easily: **"80+ body measurements"**, never "80+ body metrics", and BMI or body composition is never called a body measurement; **IEEE** only in the two approved sentences from `proof-points.md`, verbatim; no content-plan labels ("our GLP-1 hub", "this cluster", "the bridge piece"), name the topic instead; **"buyer" / "customer"** only about procurement or a signed deployment, otherwise name the actor (the clinic, the program, the employer).
