...
- [ ] **Terminology, синк 2026-09-14** (`terminology-guardrails.md` §1.10, §2.11-2.13): IEEE только двумя утверждёнными фразами из `proof-points.md`, без отдельного логотипа IEEE в полосе наград (`ieee_claims`); `80+ body measurements`, не `80+ body metrics`, и BMI / BMR / body composition не названы body measurements (`body_measurement_terms`); в H1/H2/H3, meta title и анкорах нет ярлыков контент-плана (`hub`, `cluster`, `pillar`, `bridge`, `supporting content`; Content Hub как раздел сайта можно); `buyer` только про закупку и оценку вендора, `customer` только про договор, деплой или юридическую роль, иначе назван актор
- [ ] **Abbreviations (M1 + исключение):** непонятные аббревиатуры развёрнуты при первом употреблении (DXA, GLP-1, FDA, ICH, GCP); BMI, CEO, UK, US, EU — БЕЗ расшифровки
