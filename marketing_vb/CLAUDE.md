...
> - **`brand-assets/content-strategy/terminology-guardrails.md`** — *какими словами* писать: General Approach & Language Guardrails, офлайн-копия [Doc Ассель](https://docs.google.com/document/d/1dPNXQL62t_y82MFJblBidEvRgwXjJxzADdapB7Pa214/edit) (doc изменён после 2026-08-13, синк 2026-09-14; сырой экспорт для следующего диффа — `terminology-guardrails.source.txt` рядом). Part 1 — десять правил построения фразы, Part 2 — тринадцать словарных правил, Part 3 — grep-таблица. Действует на ВЕСЬ корпоративный контент: статьи, страницы сайта, посты, outbound, whitepaper, деки. Канальных исключений нет.
>
...
2. **Read 2–3 relevant past-articles from `brand-assets/past-articles/blog/`** that match the target vertical:
   - **Any FitXpress comparison or workflow article → read `manual-vs-digital-intake-occupational-health-screening.md` first.** It is the editor's final (2026-09-11) and the current reference for sentence length, format and privacy wording. The articles listed below predate the short-sentence standard: take structure and vocabulary from them, not sentence shape.
