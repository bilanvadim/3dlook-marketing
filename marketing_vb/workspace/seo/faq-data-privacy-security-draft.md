Data, Privacy, Security & Regulatory FAQ for FitXpress[a]
FitXpress turns two smartphone photos and a short profile into structured body data: 80+ body measurements, body composition estimates, a 3D model, and progress tracking across scans. Enterprise customers integrate it through an Application Programming Interface (API) or Software Development Kit (SDK) across telehealth and weight loss, insurance underwriting, wellness, connected fitness, and clinical research. This page answers the FitXpress data privacy and security questions that procurement, legal, and security teams raise during due diligence, and sets out the product’s regulatory position; it is the reference that 3DLOOK’s AI body data for health hub and product pages point to for that detail.[b]
...
Written by Assel Sekerova. Reviewed by Legal, Security, and Product. Last updated: 2026-07-14. Procurement, legal, and security teams can request the underlying documentation through the standard procurement channel or at privacy@3dlook.me.[c]
________________
...
Topic
	...
	Backup copies are purged within a 30-day cycle. [d]A legal hold or an active security investigation can defer deletion.
	Ownership
...
Part I: [e]Data lifecycle
1. [f]What data does FitXpress process and generate?
...
* A 3D model
* Progress-tracking data that links scan results across sessions[g]
...
What trips up most procurement reviews is treating “who owns the data” as one question, when it is really five. [i]Personal-data rights stay with the end user, while contractual rights over submitted data and generated outputs sit with the enterprise customer. 3DLOOK holds only the processing rights needed to run the service, plus the intellectual property in its software and models. The generated outputs themselves are assigned to the customer by contract.[j]
________________


5. Does 3DLOOK use customer data to train AI models?[k]
...
Under GDPR, data becomes biometric data when it results from specific technical processing used to uniquely identify a person. Because FitXpress does not perform that unique identification, a body scan falls outside the biometric-data definition by default.[m]
...
11. Is FitXpress FDA approved or regulated as a medical device?[n]
...
Enterprise stakeholders can request the following through 3DLOOK’s standard procurement channel or at privacy@3dlook.me[o]:
...
[a]Inline sources are missing


The instructions required sources to appear beside the claims they support. The draft contains no public citations or linked evidence.


At minimum, source or link:
...
BAA availability
Subprocessor list
Security-documentation request route
Accuracy framework
FDA intended-use or regulatory statement
API deletion documentation, if public


For evidence available only under NDA, state that directly beside the claim.
[b]The introduction contains an unnecessary meta-reference


Remove:


It is the reference that 3DLOOK’s AI body data for health hub and product pages point to for that detail.


This describes the content architecture rather than helping the buyer. It also conflicts with the established rule against meta-references to hubs and articles.


The introduction can end after explaining the scope and governing documents.
[c]“Reviewed by Legal, Security, and Product” should not appear yet


The draft should not claim review that has not occurred.


Use an internal note:


Review required: Legal, Security, Product, Regulatory


Only publish the reviewer line after those teams have actually approved the final wording.


The same applies to the visible last-updated date. It should reflect the published or materially revised version, not the date the AI produced Draft 1.
[d]Here and throughout the text, the published Privacy Policy and draft retention claims are not aligned


The draft states that photos are deleted immediately by default, outputs are retained for the active contract term, and backups are purged within 30 days.


The current public Privacy Policy uses much broader language. It says personal data is generally reviewed after three years and may, in certain circumstances, be kept for up to another ten years. It also says data received from a third-party controller is not retained longer than necessary to fulfil the relevant contract.


These statements are not necessarily contradictory because the public policy covers more than FitXpress production data. But the relationship must be explained.
...
FitXpress production-data retention is governed by the applicable customer agreement, Data Processing Agreement, and configured product policy. The general 3DLOOK Privacy Policy covers additional categories of personal data, including website, account, commercial, and support information, which may follow different retention periods.


Do not publish a product-specific “active contract term” rule unless the DPA and technical policy actually establish it.
[e]Don't need these identifications - Par I, Part II, etc. Vary the Hn levels
[f]Don't need numerical identifications as well - 1, 2, 3. Vary the Hn levels
[g]Product-detail claims are too specific for this article
...
Smart Scales mean absolute error of 2.1 kg
Approximately 3.5% average error
All listed body composition outputs
3D overlays
Exact progress-comparison mechanics


These details create three problems:


They require technical and validation evidence.
They risk cannibalizing the accuracy framework.
They make future product changes harder because the trust article will silently become outdated.
Recommended simplification


Replace the formula and accuracy paragraph with:


Depending on the implementation, FitXpress may generate body measurements, weight- and BMI-related outputs, body composition estimates, a 3D model, and longitudinal comparison data. Body composition outputs are derived estimates rather than direct clinical measurements. Methodology and validation are covered in the body-scanning accuracy framework.


The 2.1 kg figure belongs in an accuracy or validation article, not in the data lifecycle section.
[h]Some phrasing is too conversational for a canonical trust article


Revise:


The historical records the feature needs scale to what is being compared.


Suggested:


The historical records required depend on the type of comparison.
[i]Some phrasing is too conversational for a canonical trust article


Remove or revise:


What trips up most procurement reviews is treating “who owns the data” as one question, when it is really five.


Suggested:


“Data ownership” involves several distinct rights: individual privacy rights, customer contractual rights, processing rights, intellectual-property rights, and rights to generated outputs.
[j]Ownership language must match the contract exactly


The draft says:


The generated outputs themselves are assigned to the customer by contract.


“Assigned” has a specific intellectual-property meaning. The actual contract may instead grant:


Ownership
A licence
Access and usage rights
Rights to customer data but not all generated artefacts
Different rights for measurements, meshes, reports, and platform outputs


This sentence cannot be inferred from general commercial practice.


Recommended replacement until Legal confirms it


The customer’s rights to submitted data and generated outputs are defined in the applicable customer agreement. 3DLOOK retains ownership of its software, algorithms, models, and underlying technology.
[k]The AI-training section contains several contract-level claims


The direct answer is valuable, but these statements need verification:


Explicit contractual authorization is always required.
Research and validation data are always kept distinct from production data.
Customers can prohibit training use through the DPA.
Aggregated or anonymized production data may be used for service improvement.
Technical logs are never repurposed for training.
...
DPA
Privacy Policy
Product data-use policy
Model-development policy
Actual data pipelines


Check what is available to you and apply
[l]The AWS encryption explanation contains a technical error


The draft says:


Amazon S3 Server-Side Encryption with managed keys (SSE-S3) encrypts stored data by default... Encryption keys are managed through AWS Key Management Service (KMS).


Those are two different encryption configurations.
...
AWS documents them as separate options.


Correct only after Security confirms the actual configuration


If 3DLOOK uses SSE-S3:


Data stored in Amazon S3 is encrypted using server-side encryption with Amazon S3-managed keys.


If 3DLOOK uses SSE-KMS:


Data stored in Amazon S3 is encrypted using server-side encryption with AWS KMS keys.


Do not mention both unless different data stores genuinely use different configurations and the distinction is explained.
[m]FitXpress photos and body outputs are not automatically classified as biometric data solely because they describe physical characteristics. Classification depends on whether the implementation uses specific technical processing that allows or confirms unique identification and on the purpose for which the data is processed. FitXpress is not designed as an identity-authentication system, but each deployment should be assessed under the applicable law.
[n]The FDA section is directionally good but too definitive


The section correctly emphasizes that FDA treatment depends on intended use and claims. FDA guidance confirms that labeling, promotional materials, functionality, clinical claims, and intended use all matter. Products that make diagnostic, disease-monitoring, clinical-management, or medical-equivalence claims may fall outside the general-wellness policy.


However, several statements require regulatory approval:


“FitXpress is not positioned as a medical device.”
“FitXpress has pursued none of these pathways.”
The complete list of supported uses.
“Clinical research data collection” and “eligibility documentation” as supported uses.
The assertion that progress tracking is general wellness rather than monitoring.


The word “monitoring” can itself become regulatory-sensitive depending on what is monitored and what decisions follow.


Recommended approach


Keep the section shorter:


State the approved current position.
Explain that regulatory status depends on intended use and claims.
Clarify that the product does not independently diagnose or recommend treatment.
Require customers to assess the complete integrated workflow.
Link to an approved intended-use statement.
[o]The privacy contact address is inconsistent
...
The current Privacy Policy gives:
privacy@3dlook.ai
[p]The HIPAA section needs a more balanced responsibility statement


The draft says:


Compliance depends on how the customer configures and operates its own workflow.


That may imply compliance rests mainly with the customer. Business associates also have direct obligations and may be directly liable for violations of applicable HIPAA provisions.


Recommended position


FitXpress can support HIPAA-regulated deployments where 3DLOOK acts as a business associate under an executed BAA. Both 3DLOOK and the customer remain responsible for the HIPAA obligations applicable to their respective roles, systems, and processing activities.