...
editing_passes: 5
ai_density_before: 0.55
ai_density_after: 0.0
detector_verdict: "CLEAN (hard_fails 0, house_rule_violations 0, em dashes 0, rhythm variation 0.51)"
claims_verified: [FX-001, FX-002, FX-003, FX-004, FX-006, FX-007, FX-008, FX-009, FX-010, FX-011, FX-014, FX-016]
changes_summary: |
  Pass 1 (citation dedup): accuracy framework linked once in H2.9 and referenced without a
    second link in FAQ Q3. Wellness Rewards linked twice on purpose (H2.7 handoff, then the
    routing block), which is the CTA's job on a hub page.
  Pass 2 (structure & flow): no "Furthermore / Moreover / Additionally" transitions used; H2
    bridges are thematic. Cut a dangling forward-reference to the P2 combined implementation
    guide, which does not exist yet and read as a promise.
  Pass 3 (expert voice): removed 1 banned word ("state of the art"); rewrote 3 corrective
    negations that carried no product or regulatory licence ("not wrong, they are coarse";
    "a procurement gate, not a footnote"; "the honest version of the question is not...").
  Pass 3b (strategy compliance): "What FitXpress does not do" present as its own block; no
    banned positioning claims; wellness vertical boundary held (no clinical, clearance or
    decisioning framing); internal links present in all four directions.
  Pass 3c (AI-tells + terminology): detector run twice, 0.55 -> 0.0 per 1,000 words, CLEAN.
    Terminology guardrails Part 1 and Part 2 walked as a checklist: zero "we/our/you/your",
    zero "rather than", zero bare URLs, zero em dashes, no "objective" about our own output.
  Pass 4 (final polish): every abbreviation expanded at first use (BMR, HIPAA, GDPR, API, SDK,
    DEXA, BIA) with BMI, AI, US, EU left bare per the commonly-known exception; 3 stacked
    negations split per guardrail M2, including the highest-risk sentence in H2.7; removed an
    invented illustrative figure ("waist down 3 centimetres") that was not a traceable claim;
    reframed two unsourced market observations about scan cadence as recommendations.
self_check: |
  What still read as machine-written after the first clean detector run, and what was done:
  - H2.3 was a template: four paragraphs opening with an identical short declarative fragment,
    under a sentence that promised three conditions and delivered four. Fixed the count and
    reshaped the fourth paragraph so the pattern breaks.
  - Three separate "here is the key thing" pointers ("The important word is...", "The
    interesting part is...", "This is the mechanism..."). Rewrote one.
  - The same "the honest limit is worth naming" move appeared three times across H2.2, H2.5
    and H2.6. Two rewritten; one kept, because a hub does need its boundary stated.
  - One paragraph in H2.5 had chained "and" clauses that lost the thread. Split.
  Residual, accepted: the evaluation criteria in H2.9 open in a deliberate parallel shape
  ("Capture reliability is the next filter", "Privacy belongs in...", "Integration effort is
  the last filter"). That is a checklist doing its job, and breaking it would cost clarity.
---
