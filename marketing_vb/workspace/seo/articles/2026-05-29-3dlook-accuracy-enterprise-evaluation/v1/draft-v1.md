...
phase: 3_full_draft
parent_phases: [phase-1-data-audit.md, phase-1b-pdf-reverify.md, phase-2-outline.md]
---

# How Accurate Is 3DLOOK? An Enterprise Evaluation Framework

Enterprise buyers evaluating mobile body scanning rarely get a useful answer to the question that opens every sales call: how accurate is it? The number a vendor quotes — 95%, 96%, 98%, sometimes higher — depends entirely on what they measured against, how often they measured it, and what population they tested on. Two vendors quoting the same headline percentage can perform very differently in production.
...
For an enterprise buyer, five dimensions matter together: measurement accuracy, scan-to-scan repeatability, real-world robustness when conditions are imperfect, output breadth against the actual use case, and validation strength — what external evidence backs the claims. Each of these is covered below, with the relevant numbers from 3DLOOK's internal validation work and what those numbers mean. The article closes with an eight-question evaluation checklist any buyer can take into the next vendor conversation.
...
Different verticals need different outputs. A telehealth program for GLP-1 retention needs verified BMI and longitudinal progress visualization; a uniform manufacturer needs a specific subset of girth and length measurements; a life underwriter needs BMI plus waist circumference plus a body fat estimate. The same scan event can serve any of these, but the outputs and limitations differ.
...
Where direct comparative validation against a specific reference method has not been completed, 3DLOOK should not be positioned as equivalent to that method. That is the disclosure standard the rest of this article holds to.

## How the 3DLOOK pipeline actually works

3DLOOK's mobile body scanning is powered by a proprietary statistical generative human body model — a different approach from traditional 3D reconstruction methods that depend on depth sensors, structured-light hardware, or video-based photogrammetry. The model was trained on a dataset described in the next section, and it produces a 3D body model plus 80+ measurements from two smartphone photos.
...
The body shape segmentation step is patented. The broader product is built around it, not the other way around — the segmentation engine is what allows the system to treat photos of fully clothed users as input, rather than requiring users to undress or wear tight-fitting reference garments.
...
The dataset was expanded in 2022 through a partnership with North Carolina State University's Wilson College of Textiles, publicly listed on NCSU's [3D Body Scanning research projects page](https://3dbodyscan.textiles.ncsu.edu/projects/) as a funded body scanning project with 3DLook Inc. as a funding partner. Through that collaboration, additional ground-truth 3D scans were captured using the university's Size Stream Body Scanner, with participants photographed in both casual or sports clothing and tight-fitting athletic clothing, and metadata recorded for height, weight, age, gender, ethnicity, and occupation. The collaboration extended the dataset's diversity; it did not validate 3DLOOK's measurement output, which remains an internal validation question.

The 2026 version of the dataset has continued to grow since these initial figures, but the 5,000+ / 150,000+ / 30,000+ / 430,000+ numbers reflect the documented dataset scope referenced in 3DLOOK's most recent technology presentation.
...
Different body-measurement methods exist because they answer different questions. The honest framing for 3DLOOK is to describe each method in its own terms and note what 3DLOOK is and is not designed to do alongside it.
...
For procurement teams, that distinction matters: the compliance evaluation for 3DLOOK should run on data privacy and storage frameworks (HIPAA, GDPR, SOC 2 where applicable), not on medical device frameworks (FDA Class II, CE-MDR), because the latter do not apply to the product as designed.
...
The full stack runs cross-platform on Android and iOS, from older devices through current models, without depth-sensor requirements or accessory hardware. That cross-platform footprint is a practical buyer consideration: deployment at scale across an enterprise user base typically encounters device fragmentation, and a system that only works on the most recent flagship phones produces a coverage problem.
...
No. Bioelectrical impedance analysis, including InBody, measures body composition through electrical impedance and is sensitive to hydration state. 3DLOOK estimates body composition from body shape using the U.S. Navy formula, and is designed for a different operational context — see the body composition section.
...
3DLOOK's internal validation dataset covers ages 16 to 78, heights 150 to 205 cm, weights 38 to 210 kg, US and Europe, with multi-ethnic and multi-occupation metadata. This is the disclosed population the measurement quality figures apply to, not a "works for everyone" claim. For populations outside this scope, use cases should be evaluated separately.
