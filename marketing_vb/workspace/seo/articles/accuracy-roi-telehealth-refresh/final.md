...
edited_by: seo-editor
edited: 2026-09-23
editing_passes: 5
ai_density_before: 0.0
ai_density_after: 0.0
claims_verified: [FXS-SPEED, FXS-OUTPUTS, FXS-DELIVERY, FXS-ACCURACY, FXS-REPEAT, FXS-POPULATION, FXS-SCOPE, FXS-MEDICAL, FXS-HIPAA, FXS-GDPR, FXS-RETENTION, FXS-IDS, CDC-PCD-2023]
changes_summary: |
  - Near-duplicates resolved: S7 intended-use sentence reworded (was identical to the scope note, L43~L138 1.00); FAQ 1 and FAQ 2 merged (both repeated S7 / scope note), FAQ now 3 questions; FAQ 4 no longer restates the 30-day figure and adds FXS-IDS instead; S8 bullets de-duplicated (self-report list said the same thing twice, L147~L159).
  - Repeated phrases cleared: "a guided scan" x4 -> 0 (guided mobile capture / first scan / a scan), "the current workflow" x3 -> 1; "program's clinicians" x3 and "height and weight" x3 introduced mid-pass also cleared.
  - Intro reframe rebuilt without corrective contrast ("rarely comes from X alone. It comes from Y" -> "Most of the return ... comes from Y" + one sentence placing accuracy inside it); triple "checking, chasing, or re-entering" cut to two.
  - Opening H2 renamed (it near-duplicated the S3 H2): "Patient volume and the staff time behind body data".
  - ROI section: keyword-stuffing line ("any health ROI or digital care ROI case") replaced with a caveat on one-time integration and training costs; no 3DLOOK-supplied outcome number in S5 (the only vendor input is `s`, from the program's quote).
  - Next steps sentence reworded so it no longer echoes the S8 pilot line.
  - Detector hit fixed mid-pass: "Diagnosis" (claims_discipline) in the reworded S7 sentence.
  - Kept as written: CDC PCD 2023 citation with inline link; accuracy_formulations section 5 short forms (verbatim, framework link in the same paragraph); photo deletion from compliance.md; Yazen stays cut; byline Vadim Bilan; slug unchanged.
self_check: |
  - Still machine-like after the first pass: the intro's reframe sentence ("Accuracy counts to the extent...") read as an aphorism and re-used "touch"; rewritten plainly.
  - The boundary was stated four times (scope note, short answer, S7, two FAQs) in near-identical words; now three phrasings, one FAQ.
  - The ROI section ended on a keyword line with no content; replaced by a real caveat (one-time costs, payback period).
  - S8 guided-capture bullets were two versions of "staff time is the constraint"; third bullet now names the observable symptom (patients stopping at verification).
  - Gate numbers: mean_words 11.9, over_25 0, over_35 0, near_duplicate_pairs none, repeated_phrases none; detector CLEAN 0.0/1000.
---
...
## Patient volume and the staff time behind body data
...
A telehealth or glucagon-like peptide-1 (GLP-1) weight-management program doubles its enrollment. Every self-reported height and weight that needs a second check or a re-entry becomes staff time.

Most of the return from better body data comes from fewer manual touches per patient, at a data quality the program's review workflow accepts. Accuracy matters as far as it lets a reviewer accept a record without checking it again.

That makes telehealth patient monitoring an operations question for program leaders at telehealth, digital-health, and GLP-1 or weight-management providers. The wider picture of [remote body data in telehealth workflows](https://3dlook.ai/content-hub/the-potential-of-ai-in-telehealth/) is covered in the telehealth AI overview.
...
Run both lines twice. The first run covers a baseline period on the existing process. The second covers the pilot period with the new capture step and adds `P × s` as its cost. The difference between the two totals is the return.

Reliable inputs come from records. Time a sample of verifications to set `m` and `x`. Count resubmissions in the logs to set `e`. Measure `d` at the same funnel step in both periods. The two lines leave out one-time costs such as integration and staff training. Add them to the pilot column when the case needs a payback period.
...
- **Baseline at enrollment.** The first scan sets the reference point for later comparisons.
...
Inside the program, the scan record is one input to clinician review. Clinical decisions, including treatment eligibility, remain with the care team. <!-- claim: FXS-SCOPE --> Where a protocol requires dual-energy X-ray absorptiometry (DXA), BIA, or a calibrated scale, that method stays in place. FitXpress is not a medical device. <!-- claim: FXS-MEDICAL -->
...
- The submitted values fill administrative fields only.
- No review threshold or progress comparison uses them.
...
- Clinicians compare scans over time, or a review threshold uses the result.
- Patients stop at the verification step while waiting for a call or visit.
...
No. The scan adds a structured, timestamped record to clinician review, and the prescribing clinicians decide eligibility and dosing. <!-- claim: FXS-SCOPE --> The program also sets any threshold that routes a record to that review.

### Can a program use self-reported and scan-based data together?

Yes. A common pattern collects self-reported data at sign-up and adds a scan at enrollment and at program milestones. Self-report then covers administrative fields, and the scan supports review thresholds and progress comparisons.

### What happens to the photos and the scan results?

Photos and scan results follow different rules. Photos are removed after processing, on a schedule the program's policy sets. Scan results are retained, and the program can request deletion by scan identifier. <!-- claim: FXS-RETENTION --> Stored records carry randomly generated IDs, and 3DLOOK cannot identify a specific individual from them. <!-- claim: FXS-IDS --> The [privacy and security FAQ](https://3dlook.ai/content-hub/fitxpress-data-privacy-security-regulatory-faq/) answers the detailed questions.

## Next steps

Fill in the ROI model from a timed sample of verifications and the exception logs for the same period. Then [talk to 3DLOOK about the program's body-data workflow](https://3dlook.ai/structured-body-data-for-telehealth-digital-health-programs/#bd-modal).
