...
superseded_note: "Superseded by draft-v6-revision2.md, then by the live editorial pass. Historical artefact."
created: 2026-08-01
revised: 2026-08-05
revision_note: "Revision 1 applied per review1-comments.md (12 priority items). Center of gravity moved from product-led to market-led per reviewer direction. See changelog-revision1.md for the full item-by-item mapping."
...
As patient volumes climb, the operational question shifts from getting a patient started to supporting them through months of a program. Onboarding a GLP-1 patient is comparatively straightforward; documenting and reviewing progress across a program that may run 30, 60, or 90 days, at the volumes GLP-1 programs now handle, is where the infrastructure gets tested. This hub covers the market side of that story: how fast GLP-1 programs are growing, what is driving that growth, how the delivery models built around GLP-1 differ, and what the resulting infrastructure challenge means for progress tracking.

**Short answer.** GLP-1 market growth is expanding from diabetes treatment into dedicated weight management, pulling in more delivery models and more patients per program. That growth is creating an infrastructure challenge: programs need a repeatable way to document patient progress beyond scale weight, at a volume manual methods do not support.
...
The runway extends further still. Only about 7% of people with diabetes and roughly 2% of the eligible obese population globally were using GLP-1 medications, which leaves substantial headroom as the use case widens from diabetes management into dedicated weight management.
...
Digital-health entrants show how fast programs are scaling around the medication. Hims & Hers reported $2.35 billion in total revenue for 2025, up from $1.5 billion in 2024, ended the year with 2.5 million subscribers, and projected $2.7 to $2.9 billion in 2026 ([STAT News](https://www.statnews.com/2026/02/23/him-hers-earnings-2026-outlook-compounded-semaglutide/)). These figures cover the whole company rather than the weight-management line alone, and the company named weight management as one contributing driver of that growth rather than the only one. Even read that way, the numbers point to how quickly consumer-facing GLP-1 programs have scaled into a large, competitive market.
...
**Employers and payers are participating at greater scale.** The move from 28% to 43% coverage among large employers in a single year signals that weight-loss coverage is moving from an exception toward an expected benefit.

**Virtual and hybrid care models are absorbing much of the new volume.** Telehealth and hybrid programs let a patient start and continue GLP-1 treatment without visiting a clinic for every check-in, part of how digital-health programs have scaled into a market worth billions in annual revenue.
...
**Cost and coverage pressure is following demand.** With 66% of large employers reporting a significant effect on drug spending, payers have a direct financial reason to expect programs to demonstrate that coverage produces measurable outcomes rather than prescriptions filled.
...
The first limitation is consistency. Self-reported measurements are easy to misstate, and manual progress photos are captured under different lighting, poses, and phone angles each time. There is no reliable way to line up one month against the next when the inputs keep shifting.

The second limitation is what this does to clinical review. A care team opening a patient record and finding a single weight value and a free-text note has little structured data to work from and little that is easy to verify. At the caseload sizes described above, thin and inconsistent inputs slow review down rather than speeding it up.

The third limitation is what it does to program economics and engagement. Payers and employers now covering the weight-loss indication at scale, and already seeing costs run ahead of expectations, have a direct interest in programs that can document outcomes rather than report self-reported figures. Limited progress visibility can also make it harder for programs to maintain member engagement between check-ins; the detailed evidence behind that link lives on the [Visual Progress Tracking for GLP-1](https://3dlook.ai/content-hub/visual-progress-tracking-glp1-adherence-retention/) page.
...
Scale weight and BMI are partial signals. Both compress a complex physical change into one number, and neither separates whether a change in weight came from fat, lean mass, or fluid. For a GLP-1 program, that gap matters because preserving lean mass during rapid weight loss is a recognized clinical concern, and a scale cannot show whether it is happening.

Body composition is a richer longitudinal signal than weight alone. Fat percentage, lean mass, and fat mass, tracked over successive check-ins, describe the shape of a change rather than only its size. Longitudinal body-composition estimates can give care teams additional context alongside weight, patient history, and other information used in the program, particularly during the weeks when scale weight alone looks like a plateau but underlying composition is still shifting.
...
## How Progress Tracking Fits Into Program Workflows

The way these five requirements get implemented depends on where a program sits in the ecosystem described above, but the operational pressure is consistent everywhere volumes are high.

Documentation consistency comes first. When every patient is captured through the same guided process, the record does not drift with who captured it or where, and that consistency is what makes records comparable program-wide rather than clinician by clinician.

Review workflow comes second. Structured data is faster to scan than a free-text note, which matters directly to how many patients a clinician or care coordinator can responsibly review in a shift.

Longitudinal comparison comes third, and it depends on the first two. Comparing a patient's month-three check-in to their month-one baseline only produces a meaningful signal if the capture method is consistent and the record is structured enough to compare directly.

Compliance and reporting pressure sits alongside all three. Employers and payers now expect documented outcomes rather than self-report, a pressure covered in more detail on the [GLP-1 Compliance Challenge](https://3dlook.ai/content-hub/glp-1-compliance-challenge/) page. At the workflow level covered here, the point is that documentation quality is no longer a back-office detail. It is part of how a program demonstrates it is worth the coverage it receives.

## Where FitXpress Fits

FitXpress is a structured body-data capture layer that programs can build into a GLP-1 workflow at intake and at every check-in. It gives a program a repeatable way to collect structured BMI-related and body-composition outputs from two smartphone photos, so clinical and program teams get more consistent longitudinal records to work from instead of self-report.

The capture itself is built to remove friction from a remote program. A guided scan uses two photos, front and side, and returns results in under 45 seconds. One scan produces 80+ body measurements without specialized hardware. FitXpress provides software-derived body-composition estimates, including BMI, basal metabolic rate (BMR), body fat percentage, lean mass, and fat mass. A weight estimate is also available at about ±3.5% average error, offered as a software-derived signal for context rather than a replacement for a calibrated scale.

The core of the fit for a GLP-1 program is longitudinal comparison. For most evaluated measurements, repeated scans showed typical scan-to-scan differences of less than 1 cm, which lets a program compare changes using a consistent capture method rather than reconciling differently posed photos or self-reported numbers. Accuracy is a separate question from repeatability: in internal validation against expert manual measurement, under a guided capture protocol and across a defined test population, FitXpress reported roughly 96 to 97% accuracy. The fuller framing of accurate enough for which decision, against which reference, under which protocol lives on the [mobile body scanning accuracy framework](https://3dlook.ai/content-hub/mobile-body-scanning-accuracy/) page.

FitXpress is delivered as a white-label application programming interface (API) and software development kit (SDK), so a program can build the guided scan into its own patient experience. Its privacy and regulatory posture, including how it supports HIPAA-regulated deployments and GDPR processing roles, is documented in full on the [Data, Privacy, Security & Regulatory FAQ](https://3dlook.ai/content-hub/fitxpress-data-privacy-security-regulatory-faq/). In short, photos are deleted immediately after processing by default, and 3DLOOK typically acts as a data processor or, under an executed agreement, a business associate, rather than as the data controller.

The fit is visible in production. Yazen, a weight-loss program, recorded 34,000 FitXpress scans in 2025 as part of its weight-management support workflow. In a different vertical, UK Meds uses FitXpress for BMI verification in an online-pharmacy context, covered on the [Online Pharmacy BMI Verification](https://3dlook.ai/content-hub/online-pharmacy-bmi-verification-a-2026-compliance-guide/) page. For how two photos become structured body data, see [How 3DLOOK Turns Two Photos Into Structured Body Data](https://3dlook.ai/content-hub/3dlook-turns-two-photos-structured-body-data/). This hub sits under the [AI Body Data for Health hub](https://3dlook.ai/content-hub/ai-body-data-health-hub/) and the [AI in Fitness hub](https://3dlook.ai/content-hub/ai-in-fitness-industry/).
...
As coverage and patient volumes grow, payers and employers increasingly expect programs to document outcomes rather than report self-reported figures. Scale weight alone is a thin signal that can look like a plateau even when body composition is still changing, and manual, inconsistent inputs are harder for care teams to review at scale. Structured, repeatable body data gives programs a clearer, more comparable record of what is changing over the course of treatment.
...
*FitXpress is an operational body-data capture and documentation layer. It supports review, monitoring, and documentation, and it is not a diagnostic or decisioning tool. For data, privacy, and regulatory details, see the [Data, Privacy, Security & Regulatory FAQ](https://3dlook.ai/content-hub/fitxpress-data-privacy-security-regulatory-faq/) and 3DLOOK's [legal information](https://3dlook.ai/legal/).*
