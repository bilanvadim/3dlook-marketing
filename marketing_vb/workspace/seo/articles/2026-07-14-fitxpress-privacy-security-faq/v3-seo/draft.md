...
status: draft
word_count: 3480
claims_used: [80plus-measurements, pipeline-under-45s, smart-scales-mae-2.1kg, bmr-mifflin-st-jeor, bodyfat-us-navy, photo-deletion-immediate-or-30day, face-obfuscation-at-capture, auto-blur-when-retained, tls-in-transit, sse-s3-at-rest, aws-kms, aws-us-east-1, deletion-api-user-and-scan, backup-purge-30day, no-personal-identifiers, no-data-sale, no-training-without-authorization, hipaa-baa-qualifying-plans, gdpr-processor-dpa-sccs, ccpa-service-provider, soc2-readiness-no-examination, fda-not-cleared, pen-test-annual-independent, bcdr-tested-annually]
---

# Data, Privacy, Security & Regulatory FAQ for FitXpress

FitXpress turns two smartphone photos and a small amount of profile information into structured body data: 80+ body measurements, body composition estimates, body-related metrics, a 3D model, and body and 3D model progress tracking. The platform supports telehealth, weight loss, insurance underwriting, wellness, connected fitness, and clinical research workflows, where enterprise customers integrate it through an Application Programming Interface (API) and a Software Development Kit (SDK).

This page is a general product, privacy, security, and regulatory overview for procurement, legal, and security teams. Specific behavior varies by contract, deployment, jurisdiction, integration method, and intended use. It does not replace the [Privacy Policy](https://3dlook.ai/privacy-policy/), the Terms, a Data Processing Agreement (DPA), a Business Associate Agreement (BAA), or the signed customer contract, and any of those documents governs where it differs from this summary.
...
Body composition outputs sit in a different category from body measurements. Measurements describe circumference and length at defined body points. Body composition values such as body fat percentage or lean mass are estimates, derived from those measurements together with the submitted height and optional weight. Reading every output as a "measurement" overstates what the platform produces.
...
The data-retention schedule and the subprocessor list are available through the procurement or security review process.
...
Progress tracking is off by default. Enterprise customers enable and configure progress-history retention during implementation, so a deployment that does not turn the feature on does not retain historical records for this purpose. Deleting an individual scan removes it from the progress history; deleting a full user record removes all associated scans and progress data from active systems, subject to the 30-day backup cycle described above.
...
The single question "who owns the data" collapses five rights that apply separately. Personal-data rights stay with the end user. Contractual rights over submitted data and outputs stay with the enterprise customer. Processing rights, limited to service delivery, stay with 3DLOOK. Intellectual-property rights over the software and models stay with 3DLOOK. Rights in the generated outputs are assigned to the customer by contract.
...
Under GDPR, data becomes biometric data when it results from specific technical processing used to uniquely identify a person. FitXpress does not perform unique identification, so a body scan is not automatically biometric data under that definition.

Whether an output is protected health information (PHI) under HIPAA depends on whether a covered entity or business associate holds it and whether it relates to health status, care, or payment, rather than on the data type alone.
...
The distinction is consistent across every use case: FitXpress is a supporting data layer, not the decisioning system. It improves the quality and availability of body data going into a decision, and the decision itself stays with the customer's clinicians, underwriters, or defined rules.
...
Internal security files, full audit materials, and penetration-test reports stay unpublished on this page. They are shared with qualified enterprise customers under NDA. To move a review forward, enterprise customers can request FitXpress security and compliance documentation through 3DLOOK procurement, or email privacy@3dlook.me.
