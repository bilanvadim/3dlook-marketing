...
- `closelyhq-importer` вызывает `python3 scripts/outbound-registry.py record --campaign {slug} --profile {profile}` — это заносит людей и компании в registries
- `campaign-analyzer` вызывает `python3 scripts/outbound-registry.py reply --campaign {slug} --profile {profile}` — это записывает исходы в поле `reply`

> Исправлено 2026-08-23. Раньше здесь было написано, что importer обновляет registries
> «автоматически», но в промпте importer'а не было ни одного упоминания реестров, и ни один из
> восьми outbound-агентов не ссылался на `exclusions/` вообще (двое ссылались на
> `workspace/outbound/exclusions.md` — плоский файл, которого никогда не существовало). Десять
> кампаний прошли, а реестры показывали `excluded_people: 0`. Теперь у реестра один писатель —
> `scripts/outbound-registry.py`; JSON руками не правит никто.

## Где сейчас каждая кампания

Прежде чем спрашивать Вадима «что дальше», посмотри сам:

```bash
python3 scripts/outbound-registry.py status
```

Печатает по каждой кампании: докуда дошла, сколько людей ушло в рассылку, сколько
провалидировано, и **чем именно она заблокирована**. Строки, помеченные `VADIM:`, ждут ручной
выгрузки — сам ты их не сдвинешь.

Две вещи, которые эта таблица показывает и которых иначе не видно:

- `[skipped: ...]` — кампания перескочила этап (например, `companies.csv` есть только у 3 из 10
  кампаний). Это не ошибка, но при разборе результатов важно знать, что список собирали руками.
- `⚠` — битый артефакт: import-CSV со строками, но без person-URL. Такой файл в closely.io
  бесполезен и в реестр не попадёт, его нужно перегенерировать.

Если Вадим спрашивает «что по аутбаунду» без уточнений — ответ начинается с этой таблицы, а не
с запуска новой кампании.
