...
# Trust FAQ audit 2026-09-18: sentence-level licenses must pass the live FAQ's own forms and nothing
# wider, and the live FAQ (article_type Type G) must pass the sentence gate on the Type G limits.
t  "SOC 2 definition licensed"               'echo "SOC 2 is an attestation examination performed by an independent auditor, not a product certification." | python3 brand-assets/style-guides/scripts/detect-ai-tells.py --stdin --channel article | python3 -c "import sys,json; sys.exit(0 if not json.load(sys.stdin)[\"hard_fails\"] else 1)"'
t  "reassessment boundary licensed"          'echo "A material change in intended use, including functionality intended to support diagnosis or treatment decisions, would require reassessment." | python3 brand-assets/style-guides/scripts/detect-ai-tells.py --stdin --channel article | python3 -c "import sys,json; sys.exit(0 if not json.load(sys.stdin)[\"hard_fails\"] else 1)"'
tn "independent-auditor claim still fails"   'echo "FitXpress accuracy was validated by an independent auditor." | python3 brand-assets/style-guides/scripts/detect-ai-tells.py --stdin --channel article | python3 -c "import sys,json; sys.exit(0 if not json.load(sys.stdin)[\"hard_fails\"] else 1)"'
tn "diagnosis claim still fails"             'echo "FitXpress supports diagnosis of obesity in telehealth programs." | python3 brand-assets/style-guides/scripts/detect-ai-tells.py --stdin --channel article | python3 -c "import sys,json; sys.exit(0 if not json.load(sys.stdin)[\"hard_fails\"] else 1)"'
t  "Type G limits apply to the trust FAQ"    'python3 scripts/article_lint.py workspace/seo/articles/2026-07-14-fitxpress-privacy-security-faq/published-live-2026-09-18.md --json --no-exit-code | python3 -c "import sys,json; g=[x for x in json.load(sys.stdin)[\"gates\"] if x[\"gate\"]==\"sentence length\"][0]; sys.exit(0 if g[\"ok\"] and \"Type G\" in g[\"info\"][\"limits\"] else 1)"'
t  "article limits unchanged for others"     'python3 scripts/article_lint.py $A/final.md --json --no-exit-code | python3 -c "import sys,json; g=[x for x in json.load(sys.stdin)[\"gates\"] if x[\"gate\"]==\"sentence length\"][0]; sys.exit(0 if \"article (mean <= 16\" in g[\"info\"].get(\"limits\",\"\") else 1)"'
