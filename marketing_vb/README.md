...
## Как это запускается СЕЙЧАС (2026-08-17)

Не через `telegram-bot/bot.py` из этого репо (он не работает как сервис) — а через **Hermes-бота `@dlookmarketing_bot`** и его дирижёра (conductor), который поднимает Claude Code с профилем `marketing_vb_sm` **из этого каталога**.

Три способа, все ведут в одну очередь `~/.hermes/ho.db`:

| Откуда | Как |
|---|---|
| Telegram, кнопки | бар **⚙️ Исполнитель** → **📣 Marketing VB** → 📝 Стаття · 📱 Пости зі статті · 📬 Outbound · 📣 Кампанія |
| Telegram, словом | `Стаття <тема>` · `Пости <slug>` · `Аутбаунд <рынок>` · `Кампанія <задача>` (ключевое слово — первым в сообщении) |
| Терминал / Hermes | `~/3dlook-marketing/hermes_agent/ops/mvb-run.py posts <slug>` (ещё: `article`, `outbound`, `campaign`, `articles`, `digest`, `status`) |

Что происходит дальше: job в `ho_jobs` (`profile=marketing_vb_sm`, `work_dir` = этот каталог), дирижёр запускает Claude Code с 12 плагинами, промпт = слеш-команда пайплайна (`/post-from-article` и т.д.). Вопросы и эскалации приходят в Telegram кнопками ✅/⛔/⏹, готовые посты — командой `mvb-run.py digest <slug>`.

⚠️ **`ho_jobs` руками не наполнять, `ho_steps` для маркетинга не создавать.** `ho_steps` переключает прогон в dev-верификатор (`npx ultracite lint`, `npm test`), который в контентном репо всегда падает — так job 88 закрылся как «done» с нулём постов. Дирижёр теперь такие шаги игнорирует (`HO_STEP_PROFILES`), но создавать их всё равно не надо.
